...
class EJPT_Dashboard {
    ...
    public static function ajax_get_dashboard_data() {
        ...
        if ($length === -1) { // For CSV export, fetch all
            $length = 999999; // A very large number to effectively get all
        }
        ...
        $columns = array(
            'employee_name' => 'e.last_name', // Placeholder, actual sort by first_name, last_name
            'employee_number' => 'e.employee_number',
            'job_number' => 'jl.job_number',
            'phase_name' => 'p.phase_name',
            'start_time' => 'jl.start_time',
            'end_time' => 'jl.end_time',
            'duration' => null, // Calculated, cannot sort directly on DB unless expression is built
        ...
        // Special handling for employee name sort
        if ($column_keys[$order_column_index] == 'employee_name') {
             $orderby_db_col = 'e.last_name ' . $order_dir . ', e.first_name'; // Sort by last then first
        } elseif (is_null($orderby_db_col)) {
            // For calculated columns, default to start_time or another sensible default if not sortable by DB
        ...
        $total_records = EJPT_DB::get_job_logs_count(); // Count all records without any filters for DataTables recordsTotal
